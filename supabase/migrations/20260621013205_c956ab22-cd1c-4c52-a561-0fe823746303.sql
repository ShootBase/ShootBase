...
DECLARE
  ...
  _msg_id BIGINT;
BEGIN
  RAISE LOG '[request_pro_contact] start job=% pro=% caller=%', _job_id, _professional_id, auth.uid();
  ...
  IF _existing.id IS NOT NULL THEN
    RAISE LOG '[request_pro_contact] already exists id=%', _existing.id;
  ...
  INSERT INTO public.pro_contact_requests (job_id, professional_id, customer_id, status)
    ...
    RETURNING * INTO _row;
  _was_new := TRUE;
  RAISE LOG '[request_pro_contact] record inserted id=%', _row.id;
  ...
  RAISE LOG '[request_pro_contact] notification inserted for user=%', _pro.user_id;
  ...
  IF _pro_email IS NULL THEN
    RAISE LOG '[request_pro_contact] no email on file for pro user=%', _pro.user_id;
  ELSE
    SELECT EXISTS (SELECT 1 FROM public.suppressed_emails se WHERE se.email = lower(_pro_email)) INTO _suppressed;
    IF _suppressed THEN
      RAISE LOG '[request_pro_contact] recipient suppressed email=%', _pro_email;
    ELSE
      -- Intentionally NOT wrapped in EXCEPTION so failures surface to the caller
      ...
      RAISE LOG '[request_pro_contact] email enqueued msg_id=% to=%', _msg_id, _pro_email;
    END IF;
  END IF;
